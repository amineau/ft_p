...
char *ft_signed_size(t_format *lst, int base, va_list ap)
{
	...
	else if (ft_strcmp(lst->lenght, "ll") == 0 || ft_strcmp(lst->lenght, "L") == 0)
...
{
	if (ft_strcmp(lst->lenght, "hh") == 0)
		return (ft_utoa_base((unsigned char)va_arg(ap, int), base, lst->type));
	else if (ft_strcmp(lst->lenght, "h") == 0)
		return (ft_utoa_base((unsigned short)va_arg(ap, int), base, lst->type));
	else if (ft_strcmp(lst->lenght, "l") == 0)
		return (ft_utoa_base(va_arg(ap, unsigned long), base, lst->type));
	else if (ft_strcmp(lst->lenght, "ll") == 0 || ft_strcmp(lst->lenght, "L") == 0)
		return (ft_utoa_base(va_arg(ap, unsigned long long), base, lst->type));
	else if (ft_strcmp(lst->lenght, "j") == 0)
